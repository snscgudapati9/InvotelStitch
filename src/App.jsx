...
import {
  ...
  ChevronDown, Settings as SettingsIcon, GlassWater, Martini, UtensilsCrossed, ClipboardList, Menu, Repeat, Printer, Bell
} from "lucide-react";
...
function DateRangeFilter({ from, to, setFrom, setTo, count, total }) {
  return (
    <div className="flex gap-2 items-end flex-wrap">
      <Field label="From"><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} style={{ ...selStyle, width: "auto" }} /></Field>
      <Field label="To"><input type="date" value={to} onChange={(e) => setTo(e.target.value)} style={{ ...selStyle, width: "auto" }} /></Field>
      {(from || to) && (
        <button onClick={() => { setFrom(""); setTo(""); }} className="flex items-center gap-1" style={{ background: "#fff", border: `1px solid ${COLORS.line}`, borderRadius: 4, padding: "7px 12px", fontSize: 13, cursor: "pointer", height: 34, color: COLORS.inkSoft }}>
          <X size={13} /> Clear dates
        </button>
      )}
      <span style={{ fontSize: 12, color: COLORS.inkSoft, alignSelf: "center", fontFamily: "'IBM Plex Mono', monospace" }}>{count} entries{total != null ? ` · ₹${total.toLocaleString("en-IN")}` : ""}</span>
    </div>
  );
}
...
const MODULES = [
  ...
  { key: "invoice", code: "TKT-17", label: "Multi-Item Invoice", icon: ClipboardList },
...
const ROLES = ["Owner", "Manager", "Store Manager", "Chef", "Cashier"];

const ROLE_PERMISSIONS = {
  Owner: MODULES.map((m) => m.key),
  Manager: MODULES.map((m) => m.key).filter((k) => k !== "settings"),
  "Store Manager": ["dashboard", "purchase", "invoice", "itemmaster", "stock", "indent", "store", "payables", "recurring", "barpurchase", "barstock", "cutlery"],
  Chef: ["dashboard", "wastage", "indent", "costing", "barcosting", "barwastage"],
  Cashier: ["dashboard", "sales"],
};

function ReasonPromptModal({ prompt, onClose }) {
  const [reason, setReason] = useState("");
  function submit(skip) {
    prompt.onSubmit(skip ? "" : reason.trim());
    onClose();
  }
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(38,34,32,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "#fff", borderRadius: 6, padding: 20, width: 340, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
        <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: 16, fontWeight: 600, color: COLORS.ink, marginBottom: 8 }}>Why did this change?</div>
        <div style={{ fontSize: 13.5, color: COLORS.inkSoft, lineHeight: 1.5, marginBottom: 12 }}>{prompt.message}</div>
        <input autoFocus value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g., Stock correction, physical recount" style={selStyle} />
        <div className="flex gap-2 justify-end flex-wrap mt-4">
          <button onClick={() => submit(true)} style={{ background: "#fff", border: `1px solid ${COLORS.line}`, borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}>Skip</button>
          <button onClick={() => submit(false)} style={{ background: COLORS.sage, color: "#fff", border: "none", borderRadius: 4, padding: "7px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>Save with reason</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div
      style={{
        ...
        background: `linear-gradient(90deg, ${COLORS.paperDim} 25%, #E8DFD0 37%, ${COLORS.paperDim} 63%)`,
        backgroundSize: "400% 100%",
        animation: "invotel-shimmer 1.4s ease infinite",
        ...style,
      }}
    />
  );
...
function SkeletonLoader() {
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: COLORS.paper, minHeight: "100vh" }}>
      <style>{`@keyframes invotel-shimmer { 0% { background-position: 100% 50%; } 100% { background-position: 0 50%; } }`}</style>
      <div className="flex" style={{ minHeight: "100vh" }}>
        <aside className="hidden md:flex flex-col" style={{ width: 248, background: COLORS.slate, flexShrink: 0, padding: 20 }}>
          <SkeletonBlock w="70%" h={20} style={{ marginBottom: 24, background: "linear-gradient(90deg, #3A342E 25%, #4A423A 37%, #3A342E 63%)", backgroundSize: "400% 100%" }} />
          {Array.from({ length: 8 }).map((_, i) => (
            <SkeletonBlock key={i} h={30} style={{ marginBottom: 8, background: "linear-gradient(90deg, #3A342E 25%, #4A423A 37%, #3A342E 63%)", backgroundSize: "400% 100%" }} />
          ))}
        </aside>
        <main className="flex-1" style={{ padding: 24 }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3" style={{ marginBottom: 20 }}>
            {Array.from({ length: 5 }).map((_, i) => <SkeletonBlock key={i} h={70} />)}
          </div>
          <SkeletonBlock h={180} style={{ marginBottom: 16 }} />
          <SkeletonBlock h={220} />
        </main>
      </div>
    </div>
...
export default function App() {
  ...
  const [currentRole, setCurrentRoleState] = useState(() => {
    try { return window.localStorage.getItem("invotelstitch:currentRole") || ""; } catch (e) { return ""; }
  });
  function setCurrentRole(role) {
    setCurrentRoleState(role);
    try { window.localStorage.setItem("invotelstitch:currentRole", role); } catch (e) {}
    if (role && ROLE_PERMISSIONS[role] && !ROLE_PERMISSIONS[role].includes(active)) {
      setActive("dashboard");
    }
  }
  const visibleModules = currentRole && ROLE_PERMISSIONS[currentRole] ? MODULES.filter((m) => ROLE_PERMISSIONS[currentRole].includes(m.key)) : MODULES;
  const [browserAlertsOn, setBrowserAlertsOnState] = useState(() => {
    try { return window.localStorage.getItem("invotelstitch:browserAlerts") === "true"; } catch (e) { return false; }
  });
  function setBrowserAlertsOn(on) {
    setBrowserAlertsOnState(on);
    try { window.localStorage.setItem("invotelstitch:browserAlerts", String(on)); } catch (e) {}
  }
  ...
  const [backendUrl, setBackendUrlState] = useState(() => {
    try {
      const saved = window.localStorage.getItem("invotelstitch:backendUrl");
      if (saved) return saved;
    } catch (e) {}
    // Falls back to a build-time default so every visitor to the shared link
    // connects to the same Google Sheet automatically — without this, a fresh
    // browser (or someone else opening your link) has empty localStorage and
    // sees zero data until they manually paste the URL in Settings.
    try { return import.meta.env.VITE_BACKEND_URL || ""; } catch (e) { return ""; }
  });
  const [backendStatus, setBackendStatus] = useState("idle"); // idle | ok | error
  const [isOffline, setIsOffline] = useState(() => (typeof navigator !== "undefined" ? !navigator.onLine : false));
  const [pendingSync, setPendingSync] = useState(() => {
    try { return JSON.parse(window.localStorage.getItem("invotelstitch:pendingSync") || "[]"); } catch (e) { return []; }
  });
  ...
  function markPending(key) {
    setPendingSync((prev) => {
      ...
      const next = [...prev, key];
      try { window.localStorage.setItem("invotelstitch:pendingSync", JSON.stringify(next)); } catch (e) {}
      return next;
    });
  }
  ...
  // ---- offline-first storage: always writes/reads a local mirror immediately;
  // when a Google Sheets backend is connected, it also pushes/pulls there, and
  // queues anything that fails (no internet) to retry once connectivity returns ----
  async function loadKey(key, fallback) {
    ...
    try {
      ...
      setBackendStatus("ok");
      const remoteValue = data.value != null ? JSON.parse(data.value) : null;
      if (remoteValue != null) {
        try { window.localStorage.setItem("invotelstitch:" + key, JSON.stringify(remoteValue)); } catch (e) {}
        return remoteValue;
      }
      return localValue != null ? localValue : fallback;
    } catch (e) {
  ...
  async function flushPendingSync() {
    if (!backendUrl || pendingSync.length === 0) return;
    for (const key of pendingSync) {
      try {
        const raw = window.localStorage.getItem("invotelstitch:" + key);
        if (raw == null) { clearPending(key); continue; }
        await fetch(backendUrl, { method: "POST", body: new URLSearchParams({ key, value: raw }) });
        clearPending(key);
        setBackendStatus("ok");
      } catch (e) {
        setBackendStatus("error");
  ...
  useEffect(() => {
    function goOnline() { setIsOffline(false); flushPendingSync(); }
  ...
  }, [backendUrl, pendingSync]);

  useEffect(() => {
    (async () => {
      const [p, sl, v, it, ph, s, w, ind, rp, bp, pi, bs, bw, c, a, settings, sp, savedBranches, savedRestaurantName, savedDishes, savedCocktails, savedPin] = await Promise.all([
      ...
      setBarPurchases(bp); setPurchaseInvoices(pi); setBarStock(bs); setBarWastage(bw); setCutlery(c);
      setActivity(a);
  ...
  function bulkImportDishes(rows) {
    if (!requireLogin()) return { added: 0, skipped: 0 };
    const existingNames = new Set(DISHES.map((d) => d.name));
    const grouped = {};
    rows.forEach((r) => {
      const dishName = (r.dish || "").trim();
      if (!dishName) return;
      if (!grouped[dishName]) grouped[dishName] = { name: dishName, price: Number(r.price) || 0, recipe: [] };
      const ing = (r.ingredient || "").trim();
      const qty = Number(r.qty_per_serving) || 0;
      if (ing && qty > 0) grouped[dishName].recipe.push([ing, qty]);
    });
    let added = 0, skipped = 0;
    const newDishes = [];
    Object.values(grouped).forEach((d) => {
      if (existingNames.has(d.name) || d.recipe.length === 0) { skipped++; return; }
      newDishes.push(d);
      added++;
    });
    ...
    saveKey("dishes", DISHES);
    setRecipeTick((t) => t + 1);
    logChange(`Food Costing import — ${added} new dishes added${skipped ? `, ${skipped} skipped` : ""}`);
  ...
  function bulkImportItems(rows) {
    ...
    rows.forEach((r) => {
      ...
      newOnes.push({ name, unit: r.unit || "Kg", cost: Number(r.cost) || 0, department: r.department || "Kitchen", category: r.category || "Misc", supplier: r.supplier || "" });
    });
    const next = [...items, ...newOnes];
    setItems(next);
    saveKey("items", next);
    logChange(`Item Master import — ${newOnes.length} new items added${skipped ? `, ${skipped} skipped (already existed)` : ""}`);
  ...
  function addPurchaseEntry(entry) {
    if (!requireLogin()) return;
    const amount = entry.qty * entry.rate; // excl. tax
    const cgstPercent = entry.taxMode === "split" ? Number(entry.cgstPercent) || 0 : 0;
    const sgstPercent = entry.taxMode === "split" ? Number(entry.sgstPercent) || 0 : 0;
    const gstPercent = entry.taxMode === "split" ? cgstPercent + sgstPercent : Number(entry.gstPercent) || 0;
    const gstAmount = Math.round(amount * gstPercent / 100);
    const deliveryCharge = Number(entry.deliveryCharge) || 0;
    const totalAmount = amount + gstAmount + deliveryCharge; // incl. tax + delivery — the actual invoice/payable amount
    const newRow = { ...entry, id: Date.now(), amount, gstPercent, gstAmount, deliveryCharge, totalAmount, balance: totalAmount - entry.paidAmount };
    ...
    logChange(`Purchase added — Inv#${entry.invoiceNo || "—"}, ${entry.vendor}, ${entry.item} +${entry.qty}${entry.unit} (₹${totalAmount.toLocaleString("en-IN")} incl. tax${deliveryCharge ? " & delivery" : ""}) — stock updated`);
  }
  ...
  const [reasonPrompt, setReasonPrompt] = useState(null); // { message, onSubmit(reason) }
  function requestReason(message, onSubmit) {
    setReasonPrompt({ message, onSubmit });
  }

  function setPhysicalCount(stockId, count, reason) {
    if (!requireLogin()) return;
    const row = stock.find((r) => r.id === stockId);
    const newVal = count === "" ? null : Number(count);
    const next = stock.map((r) => (r.id === stockId ? { ...r, physicalCount: newVal } : r));
    setStock(next);
    saveKey("stock", next);
    if (row && row.physicalCount !== newVal) {
      const oldText = row.physicalCount != null ? `${row.physicalCount} ${row.unit}` : "not counted";
      const newText = newVal != null ? `${newVal} ${row.unit}` : "cleared";
      logChange(`${row.item} (${row.branch}) physical count: ${oldText} → ${newText}${reason ? ` — Reason: ${reason}` : ""}`);
    }
  }
  ...
  function addPurchaseInvoice(header, lineItems) {
    if (!requireLogin()) return;
    const computedLines = lineItems.map((li) => {
      const amount = Number(li.qty) * Number(li.rate);
      const gstAmount = amount * (Number(li.gstRate) / 100);
      return { ...li, amount, gstAmount, cgst: gstAmount / 2, sgst: gstAmount / 2 };
    });
    const subtotal = computedLines.reduce((s, l) => s + l.amount, 0);
    const totalCGST = computedLines.reduce((s, l) => s + l.cgst, 0);
    const totalSGST = computedLines.reduce((s, l) => s + l.sgst, 0);
    const roundOff = Number(header.roundOff) || 0;
    const grandTotal = Math.round(subtotal + totalCGST + totalSGST + roundOff);
    const balance = grandTotal - (Number(header.paidAmount) || 0);

    const invoice = {
      id: Date.now(),
      invoiceNo: header.invoiceNo, vendor: header.vendor, branch: header.branch, billDate: header.billDate,
      lineItems: computedLines,
      subtotal: Math.round(subtotal), totalCGST: Math.round(totalCGST), totalSGST: Math.round(totalSGST),
      roundOff, grandTotal, paidStatus: header.paidStatus, paidAmount: Number(header.paidAmount) || 0,
      balance, payMode: header.payMode, payDate: header.payDate,
    };
    const nextInvoices = [invoice, ...purchaseInvoices];
    setPurchaseInvoices(nextInvoices);
    saveKey("purchaseInvoices", nextInvoices);

    let stockAcc = stock;
    computedLines.forEach((li) => {
      stockAcc = adjustStock(stockAcc, header.branch, li.item, li.unit, Number(li.qty));
    });
    setStock(stockAcc);
    saveKey("stock", stockAcc);

    logChange(`Multi-item invoice added — ${header.vendor}, Inv#${header.invoiceNo}, ${computedLines.length} items (₹${grandTotal.toLocaleString("en-IN")}) — stock updated`);
  }

  function deletePurchaseInvoice(invoice) {
    if (!requireLogin()) return;
    const nextInvoices = purchaseInvoices.filter((inv) => inv.id !== invoice.id);
    setPurchaseInvoices(nextInvoices);
    saveKey("purchaseInvoices", nextInvoices);

    let stockAcc = stock;
    invoice.lineItems.forEach((li) => {
      stockAcc = adjustStock(stockAcc, invoice.branch, li.item, li.unit, -Number(li.qty));
    });
    setStock(stockAcc);
    saveKey("stock", stockAcc);

    logChange(`Multi-item invoice deleted — Inv#${invoice.invoiceNo}, ${invoice.vendor} — stock reversed`);
  }
  ...
  useEffect(() => {
    if (!browserAlertsOn || typeof Notification === "undefined" || Notification.permission !== "granted") return;
    lowStockItems.forEach((r) => {
      const key = r.branch + "|" + r.item;
      if (!notifiedLowStockRef.current.has(key)) {
        notifiedLowStockRef.current.add(key);
        try {
          new Notification(`${restaurantName} — Low Stock`, { body: `${r.item} (${r.branch}) is down to ${r.onHand} ${r.unit}.` });
        } catch (e) {}
      }
    });
  }, [lowStockItems, browserAlertsOn, restaurantName]);
  const ownerInsights = useMemo(() => computeOwnerInsights(purchases, wastage, indents, stock, itemCostMap), [purchases, wastage, indents, stock, itemCostMap]);
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: COLORS.paper, minHeight: "100vh", color: COLORS.ink }}>
      ...
      {backendUrl && (isOffline || pendingSync.length > 0) && (
        <div style={{ background: isOffline ? "#FBF1DF" : "#EAF0EA", color: isOffline ? "#8A611C" : COLORS.sage, fontSize: 13, textAlign: "center", padding: "6px 12px" }}>
          {isOffline
            ? `You're offline — working from saved data. ${pendingSync.length > 0 ? `${pendingSync.length} change(s) will sync automatically once you're back online.` : "Changes are saving locally and will sync when back online."}`
            : `Back online — syncing ${pendingSync.length} pending change(s)…`}
        </div>
      )}
      {confirmDialog && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(38,34,32,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      ...
      {loginPrompt && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(38,34,32,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <form
            onSubmit={(e) => { e.preventDefault(); if (currentUser.trim()) setLoginPrompt(false); }}
            style={{ background: "#fff", borderRadius: 6, padding: 20, width: 320, boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
            <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: 16, fontWeight: 600, color: COLORS.ink, marginBottom: 8 }}>Who's making this change?</div>
            <div style={{ fontSize: 13.5, color: COLORS.inkSoft, lineHeight: 1.5, marginBottom: 14 }}>Set your name so edits are traceable in the activity log. This stays view-only until you do.</div>
            <input autoFocus value={currentUser} onChange={(e) => setCurrentUser(e.target.value)} placeholder="Your name" style={selStyle} />
            <div className="flex gap-2 justify-end flex-wrap mt-4">
              <button type="button" onClick={() => setLoginPrompt(false)} style={{ background: "#fff", border: `1px solid ${COLORS.line}`, borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}>Cancel</button>
              <button type="submit" disabled={!currentUser.trim()} style={{ background: currentUser.trim() ? COLORS.sage : COLORS.line, color: "#fff", border: "none", borderRadius: 4, padding: "7px 14px", fontSize: 14, fontWeight: 500, cursor: currentUser.trim() ? "pointer" : "not-allowed" }}>Continue</button>
            </div>
          </form>
        </div>
      )}
      {reasonPrompt && (
        <ReasonPromptModal prompt={reasonPrompt} onClose={() => setReasonPrompt(null)} />
      )}
      <div className="flex" style={{ minHeight: "100vh" }}>
        {/* SIDEBAR */}
        {mobileNavOpen && (
        ...
        >
          ...
          <div className="px-4 py-4" style={{ borderTop: `1px solid ${COLORS.slateSoft}` }}>
            ...
            <input
              ...
              style={{ width: "100%", background: COLORS.slateSoft, border: "none", borderRadius: 4, padding: "6px 8px", fontSize: 13.5, color: "#fff", marginBottom: 6 }}
            />
            <select
              value={currentRole}
              onChange={(e) => setCurrentRole(e.target.value)}
              style={{ width: "100%", background: COLORS.slateSoft, border: "none", borderRadius: 4, padding: "6px 8px", fontSize: 13, color: "#fff", marginBottom: 10 }}
            >
              <option value="">Role: All access (no role set)</option>
          ...
          <div className="px-4 md:px-6 py-4 md:py-5">
            {active === "dashboard" && (
              <Dashboard
                ...
                costing={costing} activity={activity} onReorder={reorderFromLastPurchase} ownerInsights={ownerInsights}
              />
            )}
            {active === "purchase" && (
            ...
            {active === "invoice" && (
              <MultiItemInvoiceModule
                rows={filterBranch(purchaseInvoices)}
                onAdd={addPurchaseInvoice}
                onDelete={(inv) => requestConfirm(`Delete Invoice #${inv.invoiceNo} — ${inv.vendor} (${inv.lineItems.length} items, ₹${inv.grandTotal.toLocaleString("en-IN")})? This will reduce Stock for all its items.`, () => deletePurchaseInvoice(inv))}
                vendors={vendors}
                items={items}
              />
            )}
            ...
            {active === "stock" && <StockModule rows={fStock} onSetPhysicalCount={setPhysicalCount} items={items} itemCostMap={itemCostMap} purchases={purchases} indents={indents} onRequestReason={requestReason} />}
            {active === "indent" && (
            ...
            {active === "costing" && <CostingModule rows={costing} sales={sales} items={items} onAddDish={addDish} onImportDishes={bulkImportDishes} onDeleteDish={(name) => requestConfirm(`Remove "${name}" from Food Costing?`, () => deleteDish(name))} />}
            {active === "wastage" && (
...
/**
 * OWNER INTELLIGENCE — turns raw transactional data into plain-language,
 * actionable observations. No ML, just disciplined comparisons:
 * month-over-month cost trend, wastage-rate trend per item, cheapest-vendor
 * comparison, and days-until-stockout projections.
 */
function computeOwnerInsights(purchases, wastage, indents, stock, itemCostMap) {
  const insights = [];
  ...
  // 1) Purchase cost trend, month over month
  const thisMonthTotal = purchases.filter((p) => monthKey(p.billDate) === thisMonth).reduce((s, p) => s + (p.totalAmount ?? p.amount), 0);
  const lastMonthTotal = purchases.filter((p) => monthKey(p.billDate) === lastMonth).reduce((s, p) => s + (p.totalAmount ?? p.amount), 0);
  if (lastMonthTotal > 0) {
    const pct = Math.round(((thisMonthTotal - lastMonthTotal) / lastMonthTotal) * 100);
    if (Math.abs(pct) >= 5) {
      insights.push({
        tone: pct > 0 ? "amber" : "sage",
        text: `Your purchase cost ${pct > 0 ? "increased" : "decreased"} ${Math.abs(pct)}% this month (₹${lastMonthTotal.toLocaleString("en-IN")} → ₹${thisMonthTotal.toLocaleString("en-IN")}).`,
      });
    }
  }

  // 2) Wastage-rate trend per item (wastage as % of total handled = wastage + indented), this month vs last
  const wasteByItemMonth = {}, indentByItemMonth = {};
  wastage.forEach((w) => { const k = w.item + "|" + monthKey(w.date); wasteByItemMonth[k] = (wasteByItemMonth[k] || 0) + w.qty; });
  indents.forEach((ind) => { const k = ind.item + "|" + monthKey(ind.date); indentByItemMonth[k] = (indentByItemMonth[k] || 0) + ind.qty; });
  const itemsSeen = new Set([...wastage.map((w) => w.item), ...indents.map((i) => i.item)]);
  itemsSeen.forEach((itemName) => {
    const wThis = wasteByItemMonth[itemName + "|" + thisMonth] || 0;
    const wLast = wasteByItemMonth[itemName + "|" + lastMonth] || 0;
    const uThis = indentByItemMonth[itemName + "|" + thisMonth] || 0;
    const uLast = indentByItemMonth[itemName + "|" + lastMonth] || 0;
    const pctThis = wThis + uThis > 0 ? (wThis / (wThis + uThis)) * 100 : null;
    const pctLast = wLast + uLast > 0 ? (wLast / (wLast + uLast)) * 100 : null;
    if (pctThis != null && pctLast != null && pctThis - pctLast >= 1) {
      insights.push({ tone: "paprika", text: `${itemName} wastage increased from ${pctLast.toFixed(1)}% to ${pctThis.toFixed(1)}% of what was used.` });
    }
  });

  // 3) Cheapest vs costliest vendor per item (only if 2+ vendors supply the same item)
  const rateByItemVendor = {};
  purchases.forEach((p) => {
    if (!rateByItemVendor[p.item]) rateByItemVendor[p.item] = {};
    if (!rateByItemVendor[p.item][p.vendor]) rateByItemVendor[p.item][p.vendor] = [];
    rateByItemVendor[p.item][p.vendor].push(p.rate);
  });
  Object.entries(rateByItemVendor).forEach(([itemName, vendorRates]) => {
    const vendorNames = Object.keys(vendorRates);
    if (vendorNames.length < 2) return;
    const avgRates = vendorNames.map((v) => ({ vendor: v, avg: vendorRates[v].reduce((a, b) => a + b, 0) / vendorRates[v].length }));
    avgRates.sort((a, b) => a.avg - b.avg);
    const cheapest = avgRates[0], costliest = avgRates[avgRates.length - 1];
    const diff = costliest.avg - cheapest.avg;
    if (diff >= 1) {
      insights.push({ tone: "ink", text: `${itemName}: ${cheapest.vendor} is ₹${diff.toFixed(0)} cheaper per unit than ${costliest.vendor}.` });
    }
  });

  // 4) Stock-out projection (avg daily usage from Total Indented ÷ 30, same approximation as Stock Inventory report)
  const indentedTotalByItemBranch = {};
  indents.forEach((ind) => { const k = ind.branch + "|" + ind.item; indentedTotalByItemBranch[k] = (indentedTotalByItemBranch[k] || 0) + ind.qty; });
  stock.forEach((r) => {
    const avgDaily = (indentedTotalByItemBranch[r.branch + "|" + r.item] || 0) / 30;
    if (avgDaily > 0) {
      const days = Math.round(r.onHand / avgDaily);
      if (days <= 5) {
        insights.push({ tone: "paprika", text: `You may run out of ${r.item} (${r.branch}) in approximately ${days} day${days === 1 ? "" : "s"}.` });
      }
    }
  });

  return insights.slice(0, 10); // keep the panel scannable
}
...
/* ---------------------------------------------------------
   DASHBOARD
--------------------------------------------------------- */
function Dashboard({ totalPurchase, totalOutstanding, totalWastageLoss, totalSalesRevenue, lowStockCount, lowStockItems, salesByDay, wastageByReason, costing, activity, onReorder, ownerInsights }) {
  return (
    <div className="space-y-5">
      {ownerInsights.length > 0 && (
        <Ticket code="OWNER INTELLIGENCE" label="What's happening in your business right now">
          <div className="space-y-2">
            {ownerInsights.map((ins, i) => (
              <div key={i} className="flex items-start gap-2" style={{ background: COLORS.paperDim, borderRadius: 4, padding: "9px 12px", borderLeft: `3px solid ${COLORS[ins.tone] || COLORS.ink}` }}>
                <span style={{ fontSize: 15, lineHeight: "20px" }}>💡</span>
                <span style={{ fontSize: 14, color: COLORS.ink, lineHeight: 1.5 }}>{ins.text}</span>
              </div>
            ))}
          </div>
        </Ticket>
...
function PurchaseModule({ rows, onAdd, onDelete, vendors, onAddVendor, items, onAddItem, prefill, onConsumePrefill }) {
  ...
  const filteredRows = filterByDateRange(rows, "billDate", dateFrom, dateTo);
  ...
  function blankPurchase() {
    const first = items[0];
    return { branch: BRANCHES[1], vendor: vendors[0] || "", item: first?.name || "", unit: first?.unit || "kg", qty: 1, rate: first?.cost || 0, invoiceNo: "", taxMode: "single", gstPercent: 5, cgstPercent: 2.5, sgstPercent: 2.5, deliveryCharge: 0, billDate: todayISO(), paidStatus: "Unpaid", paidAmount: 0, payMode: "—", payDate: "—" };
  }

  const isValidItem = items.some((i) => i.name === draft.item);

  function addRow() {
    if (!isValidItem) { setItemError(true); return; }
    onAdd(draft);
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <Ticket code="NEW ENTRY" label="Add purchase bill">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Field label="Item"><ItemPicker items={items} value={draft.item} listId="purchase-items" onSelect={(it) => { setDraft({ ...draft, item: it.name, unit: it.unit, rate: it.cost || draft.rate }); setItemError(false); }} /></Field>
            ...
            <Field label="Tax Type"><select value={draft.taxMode} onChange={(e) => setDraft({ ...draft, taxMode: e.target.value })} style={selStyle}><option value="single">Single GST %</option><option value="split">CGST + SGST</option></select></Field>
            {draft.taxMode === "single" ? (
              ...
              <>
                <Field label="CGST %"><input type="number" step="0.5" value={draft.cgstPercent} onChange={(e) => setDraft({ ...draft, cgstPercent: +e.target.value })} style={selStyle} /></Field>
                <Field label="SGST %"><input type="number" step="0.5" value={draft.sgstPercent} onChange={(e) => setDraft({ ...draft, sgstPercent: +e.target.value })} style={selStyle} /></Field>
              </>
            )}
            <Field label="Delivery Charges (₹, optional)"><input type="number" value={draft.deliveryCharge} onChange={(e) => setDraft({ ...draft, deliveryCharge: +e.target.value })} style={selStyle} /></Field>
            <Field label="Total (Incl. Tax & Delivery)">
              <input readOnly value={`₹${Math.round(draft.qty * draft.rate * (1 + (draft.taxMode === "split" ? draft.cgstPercent + draft.sgstPercent : draft.gstPercent) / 100) + (Number(draft.deliveryCharge) || 0)).toLocaleString("en-IN")}`} style={{ ...selStyle, background: COLORS.paperDim, color: COLORS.inkSoft }} />
            </Field>
          ...
          {itemError && (
            <div className="flex items-center gap-2" style={{ background: "#F5E6E2", border: `1px solid ${COLORS.paprika}`, borderRadius: 4, padding: "8px 12px", marginTop: 10, fontSize: 13.5, color: COLORS.paprikaDeep }}>
              <AlertTriangle size={14} />
              <span>"{draft.item}" isn't in the Item Master. Pick one from the suggestions, or use "Add Item" above first.</span>
            </div>
          )}
      ...
      <DateRangeFilter from={dateFrom} to={dateTo} setFrom={setDateFrom} setTo={setDateTo} count={filteredRows.length} total={filteredRows.reduce((s, r) => s + (r.totalAmount ?? r.amount), 0)} />
      <ExportBar filename="purchase_inventory_report" rows={filteredRows} title="Purchase Inventory Report" />
      <Ticket code="TKT-01" label="Purchase Inventory Report">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><Th>Invoice No</Th><Th>Branch</Th><Th>Vendor</Th><Th>Item</Th><Th right>Qty</Th><Th right>Rate (Excl.)</Th><Th>Tax</Th><Th right>Delivery</Th><Th right>Total (Incl.)</Th><Th>Bill Date</Th><Th>Status</Th><Th right>Balance</Th><Th>Mode</Th><Th>Pay Date</Th><Th></Th></tr></thead>
            <tbody>
              {filteredRows.map((r) => (
                <tr key={r.id}>
                  ...
                  <Td mono>{r.taxMode === "split" ? `CGST ${r.cgstPercent}% + SGST ${r.sgstPercent}%` : `GST ${r.gstPercent ?? 0}%`}</Td>
                  <Td right mono>{r.deliveryCharge ? `₹${r.deliveryCharge}` : "—"}</Td>
...
/* ---------------------------------------------------------
   MULTI-ITEM INVOICE MODULE — one vendor bill, many items,
   each with its own GST rate, invoice-level CGST/SGST + round-off
--------------------------------------------------------- */
const GST_RATES = [0, 5, 12, 18, 28];

function blankLineItem(items) {
  const first = items[0];
  return { item: first?.name || "", unit: first?.unit || "Kg", hsnCode: "", gstRate: 18, qty: 1, rate: first?.cost || 0 };
}

function MultiItemInvoiceModule({ rows, onAdd, onDelete, vendors, items }) {
  const [showForm, setShowForm] = useState(false);
  const [expandedId, setExpandedId] = useState(null);
  const [header, setHeader] = useState({ branch: BRANCHES[1], vendor: vendors[0] || "", invoiceNo: "", billDate: todayISO(), roundOff: 0, paidStatus: "Unpaid", paidAmount: 0, payMode: "—", payDate: "—" });
  const [lines, setLines] = useState([blankLineItem(items)]);
  const [lineError, setLineError] = useState("");

  function updateLine(i, patch) {
    setLines((prev) => prev.map((l, idx) => (idx === i ? { ...l, ...patch } : l)));
  }
  function addLineRow() { setLines((prev) => [...prev, blankLineItem(items)]); }
  function removeLineRow(i) { setLines((prev) => prev.filter((_, idx) => idx !== i)); }

  const computed = lines.map((l) => {
    const amount = Number(l.qty) * Number(l.rate);
    const gstAmount = amount * (Number(l.gstRate) / 100);
    return { ...l, amount, gstAmount, cgst: gstAmount / 2, sgst: gstAmount / 2 };
  });
  const subtotal = computed.reduce((s, l) => s + l.amount, 0);
  const totalCGST = computed.reduce((s, l) => s + l.cgst, 0);
  const totalSGST = computed.reduce((s, l) => s + l.sgst, 0);
  const grandTotal = Math.round(subtotal + totalCGST + totalSGST + (Number(header.roundOff) || 0));

  function save() {
    const invalidLine = lines.some((l) => !items.some((it) => it.name === l.item) || Number(l.qty) <= 0);
    if (invalidLine) { setLineError("Every line needs a valid item (from the list) and a qty greater than 0."); return; }
    if (!header.invoiceNo.trim()) { setLineError("Enter an Invoice No."); return; }
    onAdd(header, lines);
    setHeader({ branch: BRANCHES[1], vendor: vendors[0] || "", invoiceNo: "", billDate: todayISO(), roundOff: 0, paidStatus: "Unpaid", paidAmount: 0, payMode: "—", payDate: "—" });
    setLines([blankLineItem(items)]);
    setLineError("");
    setShowForm(false);
  }

  const exportRows = rows.map((inv) => ({
    invoiceNo: inv.invoiceNo, branch: inv.branch, vendor: inv.vendor, billDate: inv.billDate, items: inv.lineItems.length,
    subtotal: inv.subtotal, cgst: inv.totalCGST, sgst: inv.totalSGST, roundOff: inv.roundOff, grandTotal: inv.grandTotal,
    status: inv.paidStatus, balance: inv.balance,
  }));

  return (
    <div className="space-y-4">
      <div style={{ fontSize: 13, color: COLORS.inkSoft, lineHeight: 1.6 }}>
        For a single vendor bill with many items — like a real tax invoice — each item can have its own GST rate; CGST/SGST and the grand total are calculated across the whole invoice, with an optional round-off.
      </div>
      <div className="flex justify-end">
        <button onClick={() => setShowForm((s) => !s)} className="flex items-center gap-2" style={{ background: COLORS.paprika, color: "#fff", border: "none", borderRadius: 4, padding: "8px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
          <Plus size={14} /> New Multi-Item Invoice
        </button>
      </div>

      {showForm && (
        <Ticket code="NEW INVOICE" label="Add a multi-item vendor invoice">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            <Field label="Branch"><select value={header.branch} onChange={(e) => setHeader({ ...header, branch: e.target.value })} style={selStyle}>{BRANCHES.slice(1).map((b) => <option key={b}>{b}</option>)}</select></Field>
            <Field label="Vendor"><SearchableSelect options={vendors} value={header.vendor} onSelect={(v) => setHeader({ ...header, vendor: v })} placeholder="Type to search vendor…" getLabel={(v) => v} /></Field>
            <Field label="Invoice No"><input value={header.invoiceNo} onChange={(e) => setHeader({ ...header, invoiceNo: e.target.value })} placeholder="e.g., RTC/26-27/1032" style={selStyle} /></Field>
            <Field label="Bill Date"><input type="date" value={header.billDate} onChange={(e) => setHeader({ ...header, billDate: e.target.value })} style={selStyle} /></Field>
          </div>

          <div style={{ fontSize: 12.5, color: COLORS.inkSoft, marginBottom: 6 }}>Line items:</div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 10 }}>
              <thead><tr>
                <Th>Item</Th><Th>HSN/SAC</Th><Th right>GST %</Th><Th right>Qty</Th><Th right>Rate</Th><Th right>Amount</Th><Th></Th>
              </tr></thead>
              <tbody>
                {computed.map((l, i) => (
                  <tr key={i}>
                    <Td>
                      <SearchableSelect
                        ...
                        onSelect={(it) => updateLine(i, { item: it.name, unit: it.unit, rate: it.cost || l.rate })}
                      />
                    </Td>
                    <Td><input value={l.hsnCode} onChange={(e) => updateLine(i, { hsnCode: e.target.value })} placeholder="optional" style={{ ...selStyle, width: 90, padding: "6px 7px" }} /></Td>
                    <Td right>
                      <select value={l.gstRate} onChange={(e) => updateLine(i, { gstRate: +e.target.value })} style={{ ...selStyle, width: 80, padding: "6px 7px" }}>
                        {GST_RATES.map((g) => <option key={g} value={g}>{g}%</option>)}
                      </select>
                    </Td>
                    <Td right><input type="number" value={l.qty} onChange={(e) => updateLine(i, { qty: +e.target.value })} style={{ ...selStyle, width: 70, padding: "6px 7px", textAlign: "right" }} /></Td>
                    <Td right><input type="number" value={l.rate} onChange={(e) => updateLine(i, { rate: +e.target.value })} style={{ ...selStyle, width: 80, padding: "6px 7px", textAlign: "right" }} /></Td>
                    <Td right mono bold>₹{l.amount.toFixed(2)}</Td>
                    <Td><IconBtn title="Remove line" onClick={() => removeLineRow(i)}><Trash2 size={13} /></IconBtn></Td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <button type="button" onClick={addLineRow} className="flex items-center gap-1" style={{ fontSize: 13, color: COLORS.sage, background: "none", border: "none", cursor: "pointer", padding: "4px 0", marginBottom: 12 }}>
            <Plus size={14} /> Add Item Line
          </button>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3" style={{ maxWidth: 640, marginLeft: "auto", marginBottom: 10 }}>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: COLORS.inkSoft }}>Subtotal</div><div style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}>₹{subtotal.toFixed(2)}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: COLORS.inkSoft }}>CGST Output</div><div style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}>₹{totalCGST.toFixed(2)}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: COLORS.inkSoft }}>SGST Output</div><div style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}>₹{totalSGST.toFixed(2)}</div></div>
            <div>
              <div style={{ fontSize: 11, color: COLORS.inkSoft, textAlign: "right" }}>Round Off</div>
              <input type="number" step="0.01" value={header.roundOff} onChange={(e) => setHeader({ ...header, roundOff: e.target.value })} style={{ ...selStyle, textAlign: "right", padding: "4px 8px" }} />
            </div>
          </div>
          <div style={{ textAlign: "right", fontSize: 18, fontFamily: "'Oswald', sans-serif", fontWeight: 600, color: COLORS.paprika, marginBottom: 14 }}>
            Total: ₹{grandTotal.toLocaleString("en-IN")}
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <Field label="Payment Status"><select value={header.paidStatus} onChange={(e) => setHeader({ ...header, paidStatus: e.target.value })} style={selStyle}><option>Unpaid</option><option>Partial</option><option>Paid</option></select></Field>
            <Field label="Paid Amount (₹)"><input type="number" value={header.paidAmount} onChange={(e) => setHeader({ ...header, paidAmount: +e.target.value })} style={selStyle} /></Field>
            <Field label="Payment Mode"><select value={header.payMode} onChange={(e) => setHeader({ ...header, payMode: e.target.value })} style={selStyle}><option>—</option>{PAY_MODES.map((m) => <option key={m}>{m}</option>)}</select></Field>
            <Field label="Payment Date"><input type="date" value={header.payDate === "—" ? "" : header.payDate} onChange={(e) => setHeader({ ...header, payDate: e.target.value })} style={selStyle} /></Field>
          </div>

          {lineError && (
            <div className="flex items-center gap-2" style={{ background: "#F5E6E2", border: `1px solid ${COLORS.paprika}`, borderRadius: 4, padding: "8px 12px", marginTop: 10, fontSize: 13.5, color: COLORS.paprikaDeep }}>
              <AlertTriangle size={14} /><span>{lineError}</span>
            </div>
          )}
          <div className="flex gap-2 mt-3">
            <button onClick={save} className="flex items-center gap-1" style={{ background: COLORS.sage, color: "#fff", border: "none", borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}><Check size={14} /> Save Invoice</button>
            <button onClick={() => setShowForm(false)} className="flex items-center gap-1" style={{ background: "#fff", border: `1px solid ${COLORS.line}`, borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}><X size={14} /> Cancel</button>
          </div>
        </Ticket>
      )}

      <ExportBar filename="multi_item_invoices" rows={exportRows} title="Multi-Item Invoices" />
      <Ticket code="TKT-17" label="Multi-Item Invoices">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><Th>Invoice No</Th><Th>Branch</Th><Th>Vendor</Th><Th>Date</Th><Th right>Items</Th><Th right>Subtotal</Th><Th right>CGST</Th><Th right>SGST</Th><Th right>Total</Th><Th>Status</Th><Th right>Balance</Th><Th></Th></tr></thead>
            <tbody>
              {rows.map((inv) => (
                <React.Fragment key={inv.id}>
                  <tr>
                    <Td mono>{inv.invoiceNo}</Td>
                    <Td>{inv.branch}</Td><Td>{inv.vendor}</Td><Td mono>{inv.billDate}</Td>
                    <Td right mono>{inv.lineItems.length}</Td>
                    <Td right mono>₹{inv.subtotal.toLocaleString("en-IN")}</Td>
                    <Td right mono>₹{inv.totalCGST.toLocaleString("en-IN")}</Td>
                    <Td right mono>₹{inv.totalSGST.toLocaleString("en-IN")}</Td>
                    <Td right mono bold>₹{inv.grandTotal.toLocaleString("en-IN")}</Td>
                    <Td><Badge tone={inv.paidStatus === "Paid" ? "sage" : inv.paidStatus === "Partial" ? "amber" : "paprika"}>{inv.paidStatus}</Badge></Td>
                    <Td right mono color={inv.balance > 0 ? COLORS.paprika : COLORS.inkSoft}>₹{inv.balance.toLocaleString("en-IN")}</Td>
                    <Td>
                      <div className="flex gap-1">
                        <IconBtn title="View items" onClick={() => setExpandedId(expandedId === inv.id ? null : inv.id)}><ChevronDown size={13} /></IconBtn>
                        <IconBtn title="Delete" onClick={() => onDelete(inv)}><Trash2 size={13} /></IconBtn>
                      </div>
                    </Td>
                  </tr>
                  {expandedId === inv.id && (
                    <tr>
                      <td colSpan={12} style={{ background: COLORS.paperDim, padding: 10 }}>
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                          <thead><tr><Th>Item</Th><Th>HSN/SAC</Th><Th right>GST %</Th><Th right>Qty</Th><Th right>Rate</Th><Th right>Amount</Th></tr></thead>
                          <tbody>
                            {inv.lineItems.map((l, i) => (
                              <tr key={i}>
                                <Td>{l.item}</Td><Td mono>{l.hsnCode || "—"}</Td><Td right mono>{l.gstRate}%</Td>
                                <Td right mono>{l.qty} {l.unit}</Td><Td right mono>₹{l.rate}</Td><Td right mono bold>₹{l.amount.toFixed(2)}</Td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
...
--------------------------------------------------------- */
function ItemMasterModule({ items, priceHistory, onUpdate, onAdd, onImport, onDelete }) {
  ...
  function saveNewItem() {
    if (!newItem.name.trim()) return;
    onAdd(newItem.name.trim(), newItem.unit, newItem.cost, newItem.department, newItem.category, newItem.supplier);
    setNewItem({ name: "", unit: "Kg", cost: 0, department: "Kitchen", category: "Misc", supplier: "" });
  ...
  return (
    <div className="space-y-4">
      <Ticket code="IMPORT" label="Import Items from CSV">
        <div style={{ fontSize: 13, color: COLORS.inkSoft, lineHeight: 1.6, marginBottom: 10 }}>
          Columns: <b>name, unit, cost, department, category, supplier</b> (department/category/supplier optional). Existing item names are skipped, not overwritten.
        </div>
        ...
        <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2" style={{ background: COLORS.sage, color: "#fff", border: "none", borderRadius: 4, padding: "8px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
          <ClipboardList size={14} /> Upload Items CSV
        </button>
        {importResult && (
          <div className="mt-2" style={{ fontSize: 12.5, color: COLORS.sage }}>{importResult.added} items added{importResult.skipped ? `, ${importResult.skipped} skipped (already existed)` : ""}.</div>
        )}
      </Ticket>

      <div className="flex justify-end">
        <button onClick={() => setShowAddForm((s) => !s)} className="flex items-center gap-2" style={{ background: COLORS.paprika, color: "#fff", border: "none", borderRadius: 4, padding: "8px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
          <Plus size={14} /> Add New Item
        </button>
      </div>
      {showAddForm && (
        <Ticket code="NEW ITEM" label="Add a new item to Item Master">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={saveNewItem} className="flex items-center gap-1" style={{ background: COLORS.sage, color: "#fff", border: "none", borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}><Check size={14} /> Save</button>
            <button onClick={() => setShowAddForm(false)} className="flex items-center gap-1" style={{ background: "#fff", border: `1px solid ${COLORS.line}`, borderRadius: 4, padding: "7px 14px", fontSize: 14, cursor: "pointer" }}><X size={14} /> Cancel</button>
          </div>
        </Ticket>
      )}
...
/* ---------------------------------------------------------
   STOCK MODULE
--------------------------------------------------------- */
function StockModule({ rows, onSetPhysicalCount, items, itemCostMap, purchases, indents, onRequestReason }) {
                  ...
                  <Td right>
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (newVal === r.physicalCount) return;
                        onRequestReason(
                          `${r.item} (${r.branch}): ${r.physicalCount ?? "not counted"} → ${e.target.value === "" ? "cleared" : e.target.value} ${r.unit}`,
                          (reason) => onSetPhysicalCount(r.id, e.target.value, reason)
                        );
                      }}
...
function CostingModule({ rows, sales, items, onAddDish, onImportDishes, onDeleteDish }) {
  const [showForm, setShowForm] = useState(false);
  ...
  function handleFileUpload(e) {
    ...
    reader.onload = (ev) => {
      const parsed = parseCSV(ev.target.result);
      setImportResult(onImportDishes(parsed));
    };
    reader.readAsText(file);
    e.target.value = "";
  }
  ...
  return (
    <div className="space-y-3">
      <Ticket code="IMPORT" label="Import Dishes from CSV">
        <div style={{ fontSize: 13, color: COLORS.inkSoft, lineHeight: 1.6, marginBottom: 10 }}>
          Columns: <b>dish, price, ingredient, qty_per_serving, unit</b> — one row per ingredient (repeat the dish name across rows). Ingredient names should match Item Master so cost is calculated from your current prices, not a fixed number.
        </div>
        <input ref={fileInputRef} type="file" accept=".csv" onChange={handleFileUpload} style={{ display: "none" }} />
        <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2" style={{ background: COLORS.sage, color: "#fff", border: "none", borderRadius: 4, padding: "8px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
          <ClipboardList size={14} /> Upload Dishes CSV
        </button>
        {importResult && (
          <div className="mt-2" style={{ fontSize: 12.5, color: COLORS.sage }}>{importResult.added} dishes added{importResult.skipped ? `, ${importResult.skipped} skipped (already existed or no valid ingredients)` : ""}.</div>
        )}
      </Ticket>
...
function WastageModule({ rows, onAdd, onDelete, items }) {
  ...
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const filteredRows = filterByDateRange(rows, "date", dateFrom, dateTo);
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <Ticket code="NEW ENTRY" label="Log wastage / breakage">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <Field label="Branch"><select value={draft.branch} onChange={(e) => setDraft({ ...draft, branch: e.target.value })} style={selStyle}>{BRANCHES.slice(1).map((b) => <option key={b}>{b}</option>)}</select></Field>
            <Field label="Item"><ItemPicker items={items} value={draft.item} listId="wastage-items" onSelect={(it) => { setDraft({ ...draft, item: it.name, unit: it.unit }); setItemError(false); }} /></Field>
            <Field label={`Qty (${draft.unit})`}><input type="number" value={draft.qty} onChange={(e) => setDraft({ ...draft, qty: +e.target.value })} style={selStyle} /></Field>
          ...
          {itemError && (
            <div className="flex items-center gap-2" style={{ background: "#F5E6E2", border: `1px solid ${COLORS.paprika}`, borderRadius: 4, padding: "8px 12px", marginTop: 10, fontSize: 13.5, color: COLORS.paprikaDeep }}>
              <AlertTriangle size={14} />
              <span>"{draft.item}" isn't in the Item Master. Pick one from the suggestions.</span>
            </div>
          )}
          <div className="flex gap-2 mt-3">
      ...
      <DateRangeFilter from={dateFrom} to={dateTo} setFrom={setDateFrom} setTo={setDateTo} count={filteredRows.length} total={filteredRows.reduce((s, r) => s + r.estLoss, 0)} />
      <ExportBar filename="wastage_report" rows={filteredRows} title="Breakage / Wastage Report" />
      ...
      {showForm && (
        <Ticket code="NEW ENTRY" label="Issue stock to Kitchen / Bar">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Field label="Item"><ItemPicker items={items} value={draft.item} listId="indent-items" onSelect={(it) => { setDraft({ ...draft, item: it.name, unit: it.unit }); setItemError(false); }} /></Field>
            <Field label={`Qty (${draft.unit})`}><input type="number" value={draft.qty} onChange={(e) => setDraft({ ...draft, qty: +e.target.value })} style={selStyle} /></Field>
          ...
          {itemError && (
            <div className="flex items-center gap-2" style={{ background: "#F5E6E2", border: `1px solid ${COLORS.paprika}`, borderRadius: 4, padding: "8px 12px", marginTop: 10, fontSize: 13, color: COLORS.paprikaDeep }}>
              <AlertTriangle size={14} />
              <span>"{draft.item}" isn't in the Item Master. Pick an item from the suggestions (don't just type and move on) so cost and stock stay accurate.</span>
            </div>
          )}
          <div className="flex gap-2 mt-3">
...
function SalesModule({ rows, consumption, processed, onProcess, onReset, onAdd, onDelete, onImport }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <DateRangeFilter from={dateFrom} to={dateTo} setFrom={setDateFrom} setTo={setDateTo} count={filteredRows.length} total={filteredRows.reduce((s, r) => s + r.revenue, 0)} />
      <ExportBar filename="sales_daily_entries" rows={filteredRows} title="Sales — Daily Entries" />
...
--------------------------------------------------------- */
function SettingsModule({ ownerEmail, setOwnerEmail, notifyOn, setNotifyOn, activity, backendUrl, setBackendUrl, backendStatus, restaurantName, setRestaurantName, branches, onAddBranch, onClearDemoData, onBackup, onSendEmail, emailStatus, appPin, setAppPin, browserAlertsOn, setBrowserAlertsOn }) {
  ...
  function enableBrowserAlerts() {
    ...
    Notification.requestPermission().then((perm) => {
      ...
      if (perm === "granted") setBrowserAlertsOn(true);
    });
  }
  ...
  return (
    <div className="space-y-4">
      <Ticket code="ALERTS" label="Browser Notifications (free — foreground only)">
        <div style={{ fontSize: 13, color: COLORS.inkSoft, lineHeight: 1.6, marginBottom: 10 }}>
          Pops a browser notification for low-stock items while this tab is open — free, no service to pay for. It won't wake up a closed tab or send to a phone like WhatsApp would; that needs a paid messaging API later.
        </div>
        {notifPermission === "unsupported" && <Badge tone="ink">Not supported on this browser</Badge>}
        {notifPermission === "denied" && <Badge tone="paprika">Blocked — enable notifications for this site in your browser settings</Badge>}
        {notifPermission !== "unsupported" && notifPermission !== "denied" && (
          <button onClick={() => { if (browserAlertsOn) { setBrowserAlertsOn(false); } else { enableBrowserAlerts(); } }} className="flex items-center gap-2" style={{ background: browserAlertsOn ? COLORS.sage : "#fff", color: browserAlertsOn ? "#fff" : COLORS.ink, border: `1px solid ${browserAlertsOn ? COLORS.sage : COLORS.line}`, borderRadius: 4, padding: "8px 14px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
            <Bell size={14} /> {browserAlertsOn ? "Enabled — click to turn off" : "Enable low-stock alerts"}
          </button>
        )}
      </Ticket>
